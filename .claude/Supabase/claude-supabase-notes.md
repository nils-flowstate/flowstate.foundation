...
# Bereits angelegt
Ja, das Website Tracking wurde bereits umgesetzt — und zwar als eigene, datenschutzkonforme Implementierung. Hier eine Übersicht:

Hauptdatei: src/lib/analytics.ts (Zeilen 1–75)

Das sind die Kernfunktionen:

Speichert Events im localStorage (kein externer Dienst)
Trackt Session-ID via sessionStorage
Erfasst: Seitenaufruf, Scroll-Tiefe, CTA-Klicks, Formular-Abgaben, 404-Fehler
Eingebunden an folgenden Stellen:

Datei	Was wird getrackt
src/App.tsx:27	pageview bei jedem Seitenaufruf
src/hooks/useScrollDepth.ts	Scroll-Tiefe
src/components/ui/WhatsAppButton.tsx:26	WhatsApp CTA-Klicks
src/components/ui/PhoneInput.tsx:58	Formular-Abgaben
src/components/sections/HeroSection.tsx:78-83	Hero CTA-Klicks
src/pages/NotFound.tsx:13	404-Fehler
Was fehlt noch: Laut Kommentaren im Code ist eine Migration zu Supabase geplant — aktuell gehen die Daten nirgendwo hin (nur localStorage, verlassen das Gerät nicht). Ein externer Dienst wie Plausible oder Google Analytics wurde nicht eingebunden.